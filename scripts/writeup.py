...
def parse_runs(lines: list[str]) -> list[tuple[str, str, str]]:
    """Parse '## Approach' run lines: - `tool` (args_json) → ok|failed."""
    runs = []
    for l in lines:
...
def render_playbook_steps(category: str, runs: list[tuple[str, str, str]]) -> list[tuple[str, str]]:
    """Build the complete numbered step-by-step: playbook phases + recorded runs.

    Returns (kind, text) pairs: kind 'phase' (heading, unnumbered) or 'step' (numbered).
    """
    pb = PLAYBOOKS.get(category, PLAYBOOKS["misc"])
    out: list[tuple[str, str]] = []
    out.append(("step", "Read the challenge files/problem statement; note the category, platform, and any attached files."))
    for phase_title, phase_steps in pb["phases"]:
        out.append(("phase", phase_title))
        for s in phase_steps:
            out.append(("step", s))
    if runs:
        out.append(("phase", "What actually ran (recorded by the agent)"))
        for tool, args, ok in runs:
            marker = "✓" if ok == "ok" else "✗"
            detail = f" — args: {fmt_args(args)}" if args else ""
            out.append(("step", f"{marker} `{tool}`{detail}"))
    return out
...
def generate_writeup(memory_file: Path, steps: str = "") -> Path:
    ...
    # ---- Step-by-step: explicit override wins, else playbook + recorded runs ----
    if steps.strip():
        steps_lines = [("step", s) for s in steps.strip().splitlines()]
    else:
        steps_lines = render_playbook_steps(category, runs)
    ...
    used_tools = list(dict.fromkeys(t for t, _, ok in runs if ok == "ok"))
    repro = []
    for t in used_tools:
...
## 2. Step-by-step (start → finish)

{step_md}

## 3. Tools & commands

### 3.1 Terminal — {category.upper()} playbook
